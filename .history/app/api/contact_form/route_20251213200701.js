...
export async function POST(request) {
  try {
    ...
    // IP detection for Arcjet (prefer x-forwarded-for, fallback to remote address if available, or to request.headers.get("host"))
    let ip =
    ...
    let allowed = true;
    try {
      const limiterResult = await arcjetClient.limiter({
        ...limiterOptions,
        key: `contact-form-${ip}` // add a prefix for possible key collisions across services
      });

      // Strong check: Only allow if response is present and allowed is true
      if (typeof limiterResult?.allowed === "boolean") {
        allowed = limiterResult.allowed;
      } else {
        allowed = false; // fail closed if Arcjet gives a bad result
      }
    } catch (error) {
      console.error("Arcjet error", error);
      allowed = false; // block spam if Arcjet fails (fail closed for stronger DoS protection)
    }
    if (!allowed) {
      return new Response(
        JSON.stringify({
...
<div style="font-family:'Segoe UI',Arial,sans-serif;background:#f9fafb;padding:55px 0 45px 0;">
  <div style="max-width:620px;margin:auto;background:#fff;border-radius:17px;box-shadow:0 10px 40px rgba(249,203,0,0.14),0 2px 7px rgba(0,0,0,.07);overflow:hidden;">
    ...
    <div style="background:linear-gradient(to right,#F9CB00,#E2792B);color:#fff;font-size:25px;font-weight:bold;padding:18px 40px 18px 36px;text-align:center;letter-spacing:2px;">
      <span aria-hidden="true">${emailSubject}${priority ? fallbackLoader : ""}</span>
      <span style="display:none;">
        ${priority ? "High Priority Contact form received." : "New contact form submission."}
      </span>
    </div>
    <div style="padding:38px 37px 28px 37px;">
      ...
      <table style="width:100%;margin-bottom:27px;font-size:15px;background:#fcfcfc;border-radius:7px;border:1px solid #f7e7ce;box-shadow:0 0 10px #f9cb0033;">
        ...
        <tr>
          ...
          <td style="padding:8px 0;">${priority ? "High / Emergency 🚨" : "Normal"}</td>
        </tr>
      </table>
      ...
      <div style="text-align:center;margin-top:33px;">
        <span style="color:#ffd122;font-size:13.6px;letter-spacing:0.2px;">
          ...
          <b style="background-image:linear-gradient(to right,#F9CB00,#E2792B);
                    ...
                    color:transparent;-webkit-text-fill-color:transparent;">
            HimalayaFace
          </b>'s website contact form${priority ? " <b>(HIGH PRIORITY)</b>" : ""}.
        </span>
        <br />
  ...
  <div style="max-width:610px;margin:auto;background:#fff;border-radius:15px;box-shadow:0 7px 36px rgba(249,200,0,0.08),0 2px 7px rgba(0,0,0,.07);overflow:hidden;">
    ...
    <div style="padding:32px 33px 27px 33px;">
      <p style="font-size:16.6px;margin-bottom:15px;">
        Hi${name ? ` ${name}` : ""},<br/>
        Thank you for contacting&nbsp;<b style="background-image:linear-gradient(to right,#F9CB00,#E2792B);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;-webkit-background-clip:text;color:transparent;">HimalayaFace</b>.
      ...
      <div style="font-size:13px;color:#bba95a;">
        ...
        <span style="background-image:linear-gradient(to right,#F9CB00,#E2792B);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;color:transparent;">HimalayaFace</span>.
