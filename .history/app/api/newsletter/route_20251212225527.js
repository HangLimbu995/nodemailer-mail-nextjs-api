...
export async function POST(request) {
  try {
    ...
    if (!parsedData.success) {
      // If email is invalid, return explicitly
      return new Response(
        JSON.stringify({
          success: false,
          message:
            parsedData.error.formErrors &&
            parsedData.error.formErrors.length > 0
              ? parsedData.error.formErrors[0].message
    ...
    const mxValid = isValidDomain(email);
    console.log('is mxvalid is',mxValid)
    ...
    var transporter = mailTransporter("gmail", senderEmail, pass);
